...
def load_feat(d):
    ...
    node_feats_temp = None
    if os.path.exists('DATA/{}/node_features_temp.pt'.format(d)):
        node_feats_temp = torch.load('DATA/{}/node_features_temp.pt'.format(d))
        node_feats_temp = {k: v.type(torch.float32) if v.dtype == torch.bool else v for k, v in node_feats_temp.items()}

    return node_feats, edge_feats, node_feats_temp    
