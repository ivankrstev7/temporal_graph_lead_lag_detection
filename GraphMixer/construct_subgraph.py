...
#     def sample(self, n):
#         """
#         Samples `n` negative links without replacement.
#         If `n` exceeds the number of available nodes, returns all nodes.
        
#         Args:
#             n (int): Number of negative samples to generate.

#         Returns:
#             np.ndarray: Array of sampled negative links.
#         """
#         if n >= self.num_nodes:
#             return np.arange(self.num_nodes)  # Return all nodes if n exceeds available nodes
#         return np.random.choice(self.num_nodes, size=n, replace=False) 
