    ...
    def prep_input_mails(self, mfg, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                device = b.srcdata['ID'].device 
                self.node_memory = self.node_memory.to(device)
                self.node_memory_ts = self.node_memory_ts.to(device)
        ...
        with torch.no_grad():
            ...
            # APAN
                ...
                if self.memory_param['mail_combine'] == 'mean':
                    ...
                    device = nid.device  # Get the device of `nid`
                    self.mailbox = self.mailbox.to(device)  # Move mailbox to the same device
                    self.next_mail_pos = self.next_mail_pos.to(device)  # Move next_mail_pos to the same device
                    mail = mail.to(device)  # Move mail to the same device, if necessary
                    self.mailbox_ts = self.mailbox_ts.to(device)
                    mail_ts = mail_ts.to(device) 
                    # Now the operation can proceed without device mismatch
                    self.mailbox[nid.long(), self.next_mail_pos[nid.long()]] = mail
                    self.mailbox_ts[nid.long(), self.next_mail_pos[nid.long()]] = mail_ts
                else:
                    raise NotImplementedError
