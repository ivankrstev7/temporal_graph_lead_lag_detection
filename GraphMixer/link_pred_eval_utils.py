...
@torch.no_grad()
def link_pred_eval(model, args, g, df, node_feats, edge_feats, node_feats_temp=None):
    model.eval()
    ...
    max_rank=0
    max_rank_nodes = None
    max_rank_pos_embs = None
    max_rank_neg_embs = None
    rankings = None
    max_time = None
    all_positives = []
    for _, rows in cur_df.iterrows():
        ...
        if args.use_graph_structure:
            ...
            num_of_df_links = 1
            subgraph_node_feats = compute_sign_feats(node_feats, df, cur_inds, num_of_df_links, root_nodes, args, node_feats_temp)
        ...
        # if rows.time == 1701 and rows.src == 28:
        #     max_time = rows.time
        #     rankings = np.argsort(y_pred_neg)[::-1] + 1
        #     max_rank = optimistic_rank
        #     embeddings_pos, embeddings_neg = model.predict(inputs, has_temporal_neighbors, neg_samples, subgraph_node_feats, return_embeddings=True)
        #     all_positives.append(embeddings_pos.cpu().detach().numpy())
        #     max_rank_nodes = root_nodes
        #     max_rank_pos_embs = embeddings_pos
        #     max_rank_neg_embs = embeddings_neg
    ...
    # # Collect all data into a dictionary for saving
    # data_to_save = {
    #     "negative_rankings": rankings,
    #     "max_rank": max_rank,
    #     "max_time": max_time,
    #     "max_rank_nodes": max_rank_nodes,
    #     "max_rank_pos_embs": np.array(all_positives),  # Convert to numpy
    #     "max_rank_neg_embs": max_rank_neg_embs.cpu().detach().numpy(),  # Convert to numpy
    # }
