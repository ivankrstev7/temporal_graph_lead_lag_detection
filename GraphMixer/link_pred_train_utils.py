...
def run(model, optimizer, args, subgraphs, df, node_feats, edge_feats, mode, node_feats_temp=None):
    time_aggre = 0
    ...
    for ind in range(len(train_loader)):
        ...
        if args.use_graph_structure:
            ...
            subgraph_node_feats = compute_sign_feats(node_feats, df, cur_inds, num_of_df_links, subgraph_data['root_nodes'], args, node_feats_temp)
...
def link_pred_train(model, args, g, df, node_feats, edge_feats, node_feats_temp=None):
    ...
    for epoch in range(args.epochs):
        ...
        train_ap, train_auc, train_loss = run(model, optimizer, args, train_subgraphs, df, 
                                              node_feats, edge_feats, mode='train', node_feats_temp=node_feats_temp)
        with torch.no_grad():
            ...
            valid_ap, valid_auc, valid_loss = run(copy.deepcopy(model), None, args, valid_subgraphs, df, 
                                                  node_feats, edge_feats, mode='valid', node_feats_temp=node_feats_temp)
            # second variable (optimizer) is only required for training
            test_ap,  test_auc,  test_loss  = run(copy.deepcopy(model), None, args, test_subgraphs,  df, 
                                                  node_feats, edge_feats, mode='test', node_feats_temp=node_feats_temp)  
...
def compute_sign_feats(node_feats, df, start_i, num_links, root_nodes, args, node_feats_temp=None):
    ...
    for _root_ind in root_inds:
        ...
        if i == 0 or args.structure_hops == 0:
            ...
            if node_feats_temp:
                prev_row = cur_df.iloc[-1]  # This is the last row in the sliced DataFrame

                # Get the value in the 'time' column of the previous row
                prev_time = prev_row['time']
                sign_feats = [node_feats_temp[prev_time]]
            else:
