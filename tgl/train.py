...
def eval(mode='val'):
    ...
    if mode == 'val':
        ...
        batch_sizee = train_param['batch_size']
    elif mode == 'test':
        eval_df = df[val_edge_end:]
        neg_samples = args.eval_neg_samples
        batch_sizee = 1
    elif mode == 'test_one_sample':
        eval_df = df[val_edge_end:]
        batch_sizee = 1
    elif mode == 'train':
    ...
    with torch.no_grad():
        total_loss = 0
        flag = 1
        for _, rows in eval_df.groupby(eval_df.index // batch_sizee):
            ...
            y_pred_pos = pred_pos.sigmoid().cpu()
            y_pred_neg = pred_neg.sigmoid().cpu()
...
for e in range(train_param['epoch']):
    ...
    for _, rows in df[:train_edge_end].groupby(group_indexes[random.randint(0, len(group_indexes) - 1)]):
        ...
        loss.backward()
        optimizer.step()
        train_ap, train_auc = compute_ap_score(pred_pos, pred_neg, neg_samples=1)
    ...
    train_ap = sum(all_train_ap)/len(all_train_ap)
    train_auc = sum(all_train_auc)/len(all_train_auc)
    if e > 2 and ap > best_ap:
        best_e = e
        best_ap = ap
        torch.save(model.state_dict(), path_saver)
    print('\ttrain loss:{:.4f} train ap:{:4f}  train auc:{:4f}  val loss:{:4f} val ap:{:4f}  val auc:{:4f}'.format(total_loss, train_ap, train_auc, val_loss, ap, auc))
...
# else:
print('\taverage test precision:{:4f}  test AUC:{:4f}'.format(ap, auc))
