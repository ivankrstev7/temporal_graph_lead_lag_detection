...
class EdgePredictor_per_node(torch.nn.Module):
    ...
    def forward(self, h, neg_samples=1, return_embeddings=False):
        ...
        if return_embeddings:
            return h_pos_edge, h_neg_edge  # Return embeddings directly
...
class Mixer_per_node(nn.Module):
    ...
    def predict(self, model_inputs, has_temporal_neighbors, neg_samples, node_feats, return_embeddings=False):
        ...
        if return_embeddings:  # Return embeddings if requested
          pred_pos, pred_neg = self.edge_predictor(x, neg_samples=neg_samples, return_embeddings=True)
